...
/*
 * aead_copy_aad - copy associated data from TX SGL to RX SGL
 *
 * memcpy_sglist() is not available in this kernel version; implement the
 * same SG-to-SG copy using scatterwalk_map_and_copy via a temporary buffer.
 */
static int aead_copy_aad(struct sock *sk, struct scatterlist *dst,
			 struct scatterlist *src, size_t len)
{
	void *buf;
	...
	buf = sock_kmalloc(sk, len, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	scatterwalk_map_and_copy(buf, src, 0, len, 0);	/* src SG -> buf */
	scatterwalk_map_and_copy(buf, dst, 0, len, 1);	/* buf -> dst SG */

	sock_kfree_s(sk, buf, len);
	return 0;
}
			 ...
			 size_t ignored, int flags)
{
	...
	err = aead_copy_aad(sk, rsgl_src, tsgl_src, ctx->aead_assoclen);
	if (err)
		goto free;
...
static int aead_accept_parent_nokey(void *private, struct sock *sk)
{
	...
	struct alg_sock *ask = alg_sk(sk);
	struct crypto_aead *aead = private;
...
{
	if (crypto_aead_get_flags(private) & CRYPTO_TFM_NEED_KEY)
		return -ENOKEY;
